...
backup () {
    ...
    if [ -z $db_container ]; then
        ...
        docker exec $db_container sh -c 'PGPASSWORD="$POSTGRES_PASSWORD" pg_dump -U "$POSTGRES_USER" "$POSTGRES_DB"' > "$dest_path/tmp/firefly_db.sql"
    ...
    if [ -z $upload_volume ]; then
        ...
        docker run --rm -v "$upload_volume:/tmp" -v "$dest_path/tmp:/backup:z" alpine tar -czf "/backup/firefly_upload.tar.gz" -C "/" "tmp"
...
restore () {
    ...
    docker run --rm -v "$upload_volume:/recover" -v "$src_path/tmp:/backup:z" alpine tar -xf /backup/firefly_upload.tar.gz -C /recover --strip 1
    ...
    if [ -z $db_container ]; then
        ...
        cat "$src_path/tmp/firefly_db.sql" | docker exec -i $db_container psql -U firefly firefly
